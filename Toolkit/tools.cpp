...
lidarMarker Tools::lidarSense(Car &car,
  ...
  lidarMarker marker =
      lidarMarker(car.getPosition().x + noise(0.15, timestamp),
                  car.getPosition().y + noise(0.15, timestamp + 1));
  if (visualize) {
    viewer->addSphere(pcl::PointXYZ(marker.x, marker.y, 3.0), 0.5, 1, 0, 0,
                      car.getName() + "_lmarker");
  }
...
radarMarker Tools::radarSense(Car &car, Car ego,
                              pcl::visualization::PCLVisualizer::Ptr &viewer,
                              long long timestamp, bool visualize) {
  ...
  double rho = sqrt((car.getPosition().x - ego.getPosition().x) *
                        (car.getPosition().x - ego.getPosition().x) +
                    (car.getPosition().y - ego.getPosition().y) *
                        (car.getPosition().y - ego.getPosition().y));
  double phi = atan2(car.getPosition().y - ego.getPosition().y,
                     car.getPosition().x - ego.getPosition().x);
  double rhoDot = (car.getVelocity() * cos(car.getAngle()) * rho * cos(phi) +
                   car.getVelocity() * sin(car.getAngle()) * rho * sin(phi)) /
                  rho;

  radarMarker marker = radarMarker(rho + noise(0.3, timestamp + 2),
                                   phi + noise(0.03, timestamp + 3),
                                   rhoDot + noise(0.3, timestamp + 4));

  if (visualize) {
    viewer->addLine(
        pcl::PointXYZ(ego.getPosition().x, ego.getPosition().y, 3.0),
        pcl::PointXYZ(ego.getPosition().x + marker.rho * cos(marker.phi),
                      ego.getPosition().y + marker.rho * sin(marker.phi), 3.0),
        1, 0, 1, car.getName() + "_rho");
    viewer->addArrow(
        pcl::PointXYZ(ego.getPosition().x + marker.rho * cos(marker.phi),
                      ego.getPosition().y + marker.rho * sin(marker.phi), 3.0),
        pcl::PointXYZ(ego.getPosition().x + marker.rho * cos(marker.phi) +
                          marker.rhoDot * cos(marker.phi),
                      ego.getPosition().y + marker.rho * sin(marker.phi) +
                          marker.rhoDot * sin(marker.phi),
                      3.0),
...
void Tools::trackerResults(Car car,
                           pcl::visualization::PCLVisualizer::Ptr &viewer,
                           double time, int steps) {

  Tracker tracker = car.getTracker();

  viewer->addSphere(pcl::PointXYZ(tracker.x_[0], tracker.x_[1], 3.5), 0.5, 0, 1,
                    0, car.getName() + "_tracker");
  ...
  if (time > 0) {
    ...
    while (ct <= time) {
      ...
      viewer->addSphere(pcl::PointXYZ(tracker.x_[0], tracker.x_[1], 3.5), 0.5,
                        0, 1, 0, car.getName() + "_ukf" + std::to_string(ct));
      ...
      viewer->addArrow(
          pcl::PointXYZ(tracker.x_[0], tracker.x_[1], 3.5),
          pcl::PointXYZ(tracker.x_[0] + tracker.x_[2] * cos(tracker.x_[3]),
                        tracker.x_[1] + tracker.x_[2] * sin(tracker.x_[3]),
                        3.5),
...
void Tools::savePcd(const typename pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud,
                    std::string filename) {
