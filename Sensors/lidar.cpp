...
void Ray::rayCast(const std::vector<Car>& cars, double minDistance, double maxDistance, pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud, double slopeAngle, double stdErr) {
  castPosition = origin;
  castDistance = 0;
...
Lidar::Lidar(std::vector<Car> setCars, double setGroundSlope)
  ...
  //TODO increase the number of layers to 8 to get a higher resolution pcd
  int numLayers = 64;
  double steepestAngle = - 24.8 * M_PI / 180;
  double angleRange = 26.8 * M_PI / 180;
  //TODO set to pi/64 to get a higher resolution pcd
  double horizontalAngleInc = M_PI / 180;
  double angleIncrement = angleRange/ numLayers;

  for (double angleVerical = steepestAngle; angleVerical < steepestAngle + angleRange; angleVerical += angleIncrement) {
...
void Lidar::updateCars(std::vector<Car> setCars) {
  cars = setCars;
}
