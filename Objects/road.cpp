...
Road::Road() : arenaRadius(50.0), laneWidth(5.0), laneCount(2), poleRadius(0.5), poleHeight(3.0), poleSpacing(10.0) {
  computeCoefficients();
  computeDummies();
}
...
void Road::renderPoles(pcl::visualization::PCLVisualizer::Ptr& viewer) const {
  for (size_t i = 0; i < poleCoefficients.size(); ++i) {
    ...
    viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, 1, 0.5, 0, "pole" + std::to_string(i)); // Orange poles
  }
}
