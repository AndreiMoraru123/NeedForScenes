...
Car::Car()
    : position(Vect3(0, 0, 0)), dimensions(Vect3(0, 0, 0)),
      ...
      steering(0), rollingInstance(0), frontCenterDistance(0), control_index(0),
      sinNegAngle(0), cosNegAngle(0), air_resistance(0.0), tire_friction(0.0) {}

Car::Car(Vect3 setPosition, Vect3 setDimensions, Color setColor, float setAngle,
...
void Car::renderWheels(pcl::visualization::PCLVisualizer::Ptr &viewer) const {
  for (int i = -1; i <= 1; i += 2) {
    for (int j = -1; j <= 1; j += 2) {
      ...
      viewer->addCylinder(coeffs, name + "Wheel" + std::to_string(i) +
                                      std::to_string(j));
      viewer->setShapeRenderingProperties(
...
void Car::renderWindshields(
    pcl::visualization::PCLVisualizer::Ptr &viewer) const {
  for (int i = -1; i <= 1; i += 2) {
    Eigen::Vector3f globalPosition = generateWindshieldPosition(i);
    viewer->addCube(globalPosition, orientation, dimensions.x / 4, dimensions.y,
                    dimensions.z / 6, name + "Windshield" + std::to_string(i));
    viewer->setShapeRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_COLOR, 0.3, 0.7, 1.0,
...
void Car::accelerate(float acc, int dir) {
  acceleration = dir * acc - air_resistance * velocity * abs(velocity) -
                 tire_friction * sign(velocity);
}

void Car::steer(float s) { steering = s; }
...
[[nodiscard]] bool Car::checkCollision(Vect3 point) const {
  double xPrime = ((point.x - position.x) * cosNegAngle -
                   (point.y - position.y) * sinNegAngle) +
                  position.x;
  double yPrime = ((point.y - position.y) * cosNegAngle +
                   (point.x - position.x) * sinNegAngle) +
                  position.y;
