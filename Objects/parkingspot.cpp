...
ParkingSpot::ParkingSpot(const pcl::PointXYZ &position, double length,
                         double width, double height, int id)
    : position_(position), length_(length), width_(width), height_(height),
      id_(id) {}

[[maybe_unused]] pcl::PointXYZ ParkingSpot::getPosition() const {
  return position_;
}
...
void ParkingSpot::render(pcl::visualization::PCLVisualizer::Ptr &viewer) const {
  viewer->addCube(position_.x - length_ / 2, position_.x + length_ / 2,
                  ...
                  position_.z, position_.z + height_, 1.0, 1.0, 0.0, name_);
...
bool ParkingSpot::isCarParked(Car &car) const {
  ...
  bool withinX = (carPosition.x >= (position_.x - length_ / 2)) &&
                 (carPosition.x <= (position_.x + length_ / 2));
  bool withinY = (carPosition.y >= (position_.y - width_ / 2)) &&
                 (carPosition.y <= (position_.y + width_ / 2));
  bool withinZ = (carPosition.z >= position_.z) &&
                 (carPosition.z <= (position_.z + height_));
